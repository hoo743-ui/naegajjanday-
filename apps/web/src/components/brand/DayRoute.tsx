...
export function DayRoute({ stops, className, dense = false }: { stops: DayRouteStop[]; className?: string; dense?: boolean }) {
  return (
    <ol className={cn("grid sm:auto-cols-fr sm:grid-flow-col", className)}>
      {stops.map((stop, i) => (
        <li key={stop.key} className={cn("relative grid content-start pl-8 sm:pl-0 sm:pr-4", i < stops.length - 1 && (dense ? "pb-3.5 sm:pb-0" : "pb-5 sm:pb-0"))}>
          {/* 다음 곳까지의 이동: 세로(모바일) · 가로(넓은 화면) 점선 */}
          {i < stops.length - 1 ? (
            <span aria-hidden className="absolute top-4 bottom-0 left-[5px] border-l-2 border-dashed border-ink/25 sm:top-[5px] sm:right-0 sm:bottom-auto sm:left-4 sm:border-t-2 sm:border-l-0" />
          ) : null}
          <span aria-hidden className={cn("absolute top-1.5 left-0 size-3 rounded-full border-2 border-ink bg-paper sm:static sm:mb-3", i === 0 && "bg-ink")} />
