  ...
  return (
    <PageShell className="wrap-narrow section-y">
      <article className="mx-auto max-w-[760px]">
        {LEGAL_READY ? null : (
          <p role="note" className="mb-8 rounded-2xl bg-paper-2 text-ink-2 px-5 py-4 text-body-sm font-semibold">
        ...
        <p className="tabular mt-3 text-body-sm font-bold text-muted-foreground">시행일 {field(OPERATOR.effectiveDate)}</p>
        <div className="mt-6 text-body text-ink-2">{lead}</div>
        ...
        <ol className="mt-10 grid gap-9">
          {sections.map((section, i) => (
            <li key={section.title}>
              ...
              <div className="mt-3 grid gap-2.5 text-body text-ink-2 [&_li]:ml-5 [&_li]:list-disc [&_table]:w-full [&_td]:border-b [&_td]:py-2.5 [&_td]:pr-3 [&_td]:align-top [&_th]:border-b-2 [&_th]:py-2 [&_th]:pr-3 [&_th]:text-left [&_th]:text-body-sm [&_th]:font-semibold [&_th]:text-muted-foreground">
                {section.body}
              </div>
            </li>
          ))}
        ...
        <p className="mt-12 border-t pt-6 text-body-sm text-muted-foreground">
          함께 읽어 보세요:{" "}
