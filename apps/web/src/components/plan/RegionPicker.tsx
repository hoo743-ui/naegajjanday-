...
export function RegionPicker({ value, onChange }: RegionPickerProps) {
  ...
  const matches = useMemo(
    () => (query ? (regions.data?.items ?? []).filter((r) => r.name.includes(query) && r.place_count > 0).sort((a, b) => b.level - a.level || b.place_count - a.place_count).slice(0, 12) : []),
    [query, regions.data],
  );
        ...
        {query ? (
          ...
          <>
            ...
            <nav aria-label="지역 단계" className="mb-3 flex flex-wrap items-center gap-1 text-sm font-semibold text-muted-foreground">
              <button type="button" onClick={() => setPath([])} className={cn("rounded-lg px-2 py-1 hover:bg-ink/[0.05] hover:text-ink", path.length === 0 && "text-ink")}>
                전국
              </button>
              {path.map((node, i) => (
                    ...
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="tabular block truncate text-[13px] text-muted-foreground">
                          {node.children.length}곳으로 나뉘어요 · 장소 {num(node.placeCount)}곳
                        </span>
                      </span>
