...
export function PerformanceCard({ at, startAt, durationMin }: PerformanceCardProps) {
  ...
  return (
    <section aria-labelledby="performance-card" className="rule-section gap-3">
      <h2 id="performance-card" className="flex items-center gap-2 text-body font-extrabold text-ink">
      ...
      <ul className="grid gap-2">
        {shows.data.items.slice(0, 5).map((show) => (
          <li key={show.id} className="rounded-2xl border border-line p-3">
            <a href={show.detail_url ?? undefined} target="_blank" rel="noreferrer" onClick={() => track("performance_clicked", { performance_id: show.id })} className="block text-body font-extrabold text-ink hover:text-blue-deep">
              {show.title}
            </a>
            <p className="tabular mt-0.5 text-caption text-muted-foreground">
              {show.venue.name} · {distance(show.venue.distance_m)} · {show.show_times.join(", ")} 시작
              {show.runtime_min ? ` · ${show.runtime_min}분` : ""}
            </p>
            {show.price_text ? <p className="mt-0.5 text-caption text-ink-2">{show.price_text}</p> : null}
          </li>
        ))}
      </ul>
