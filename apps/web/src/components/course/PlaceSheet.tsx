    ...
    <Sheet open onOpenChange={(open) => (open ? undefined : onClose())}>
      <SheetContent side="right" className="w-full gap-0 overflow-y-auto p-0 sm:max-w-[440px]">
          ...
          <div className="flex shrink-0 snap-x snap-mandatory gap-1 overflow-x-auto bg-soft" aria-label={`${place.name} 사진 ${photos.length}장`}>
            {photos.map((url) => (
              <span key={url} className="relative block aspect-[4/3] w-[86%] shrink-0 snap-center first:ml-0">
                ...
                {photoCredit(url) ? <span className="absolute right-2 bottom-2 rounded-full bg-black/55 px-2 py-0.5 text-[10.5px] font-semibold text-white">{photoCredit(url)}</span> : null}
              </span>
            ))}
          </div>
        ) : null}
        ...
        <SheetHeader className="gap-1 px-5 pt-5 pb-0 text-left">
          <SheetTitle className="text-[21px] leading-tight font-extrabold tracking-tight">{place.name}</SheetTitle>
        ...
        <div className="grid gap-5 px-5 pt-4 pb-8">
          {d?.since_year && years >= 3 ? (
            <p className="tabular rounded-2xl bg-blue-soft px-4 py-3 text-[14px] font-bold text-blue-deep">
          ...
          {d && d.marks.length > 0 ? (
            <section aria-label="공적 표식" className="grid gap-1.5">
              {d.marks.map((m) => (
                <p key={m.tag} className="flex items-start gap-2 text-[14px]">
          ...
          {d && d.menus.length > 0 ? (
            <section aria-labelledby="place-menu" className="receipt-wrap rounded-2xl border border-line p-4">
              <h3 id="place-menu" className="flex items-center gap-1.5 text-[13px] font-extrabold text-muted-foreground">
                <Utensils aria-hidden className="size-3.5" /> 조사된 메뉴판 가격
              </h3>
              <ul className="tabular mt-2 grid gap-1.5">
                {d.menus.slice(0, 8).map((m) => (
                  <li key={`${m.name}-${m.price}`} className="flex items-baseline gap-2 text-[14.5px]">
            ...
            <p className="rounded-2xl bg-soft px-4 py-3 text-[13px] leading-relaxed text-ink-2">
          ...
          {hours.length > 0 ? (
            <section aria-labelledby="place-hours">
              <h3 id="place-hours" className="flex items-center gap-1.5 text-[13px] font-extrabold text-muted-foreground">
                <Clock aria-hidden className="size-3.5" /> 영업시간
              </h3>
              <ul className="tabular mt-1.5 grid grid-cols-2 gap-x-4 gap-y-0.5 text-[13.5px] text-ink-2">
                {hours.map((h) => (
                  <li key={h.dow}>
          ...
          {d?.description ? <p className="text-[14px] leading-[1.75] whitespace-pre-line text-ink-2">{d.description}</p> : null}

          <div className="grid gap-2">
            {d?.phone ? (
              <a href={`tel:${d.phone.replace(/[^0-9+]/g, "")}`} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-line px-4 py-3 text-[15px] font-bold text-ink hover:border-blue-deep">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-ink px-4 py-3 text-[15px] font-bold text-white hover:opacity-90"
            >
