 ...
 *  - 수집 항목: OAuth(apps/api auth) · 취향(me/preferences) · 저장 코스 · 접속 IP(rate limit) · 페이지 방문(POST /visits, docs/50)
 *    · 코스 화면 이벤트(POST /events, docs/62 — 180일, event_retention_days)
        ...
        <div className="overflow-x-auto">
          <table>
            ...
            <tbody>
              ...
              <tr>
                <td>페이지를 볼 때 (비회원 포함)</td>
                <td>본 페이지 주소, 들어온 사이트 이름, 기기 종류(모바일 · PC), 이 브라우저의 임의 번호(해시로만 저장), 접속 IP, 로그인한 경우 계정</td>
                <td>날짜별 이용자 수를 세어 서비스를 고치고, 부정 이용을 확인하기 위해 (외부 분석 업체로 보내지 않아요)</td>
              </tr>
              <tr>
                <td>코스 화면을 쓸 때 (비회원 포함)</td>
                <td>누른 기능의 이름(저장 · 공유 · 길찾기 · 장소 바꾸기 등)과 짧은 값(몇 번째 장소, 고른 옵션 종류), 코스 번호, 시각, 페이지 주소, 위와 같은 브라우저 임의 번호(해시로만 저장), 로그인한 경우 계정. 직접 입력한 글과 접속 IP 는 저장하지 않아요</td>
                <td>만든 코스가 실제로 쓰이는지 세어 추천을 고치기 위해 (외부 분석 업체로 보내지 않아요)</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  },
  {
    ...
    body: (
      <ul>
        ...
        <li>코스 화면 이용 기록(누른 기능): <b>180일</b> 뒤 자동으로 지워요.</li>
        <li>페이지 방문 기록: 최대 1년 보관한 뒤 자동으로 지워요. 방문 · 코스 요청에 남은 접속 IP 는 <b>90일</b> 뒤 자동으로 지워요.</li>
  ...
  },
  {
    ...
    body: (
      <ul>
        ...
        <li>
          이용 통계는 외부 도구 없이 직접 세요: 브라우저 저장소에 임의 번호 하나(<code>njd.visitor</code>)를 두고, 페이지를 볼 때 그 번호를 서버로 보내요. 서버는 번호를 되돌릴 수 없는 해시로만 저장해요. 브라우저 저장소를 지우면 새 번호가 돼요. 방문 기록과 접속 IP 는 운영자만 볼 수 있어요.
        </li>
        <li>
          코스 화면에서 누른 기능도 같은 번호로 우리 서버에만 보내요. 브라우저의 &lsquo;추적 안 함(Do Not Track)&rsquo;이나 &lsquo;Global Privacy Control&rsquo;을 켜 두면 보내지 않고, 서버도 그 신호가 붙은 기록은 저장하지 않아요.
        </li>
        <li>외부 이용 통계 도구를 쓰게 되면 어떤 도구인지, 무엇을 보내는지 이 문서에 먼저 적을게요.</li>
      </ul>
    ),
  },
  {
