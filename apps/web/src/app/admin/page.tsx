...
export default function AdminDashboardPage() {
  ...
  const pendingByRegion = regions.data?.items.reduce((sum, r) => sum + r.pending_count, 0);
  const pendingCount = pending.data ? (pending.data.total ?? pendingByRegion ?? pending.data.items.length) : undefined;
  const pendingIsFloor = Boolean(pending.data?.next_cursor) && pending.data?.total === undefined && pendingByRegion === undefined;
  const collecting = regions.data?.items.filter((r) => r.status === "collecting").length ?? 0;
  // 사용자 분석은 501(제품 분석 도구 연동 전)일 수 있다 — 고장이 아니라 아직 없는 통계다
  const usersNotReady = isNotReady(users.error);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <StatCard label="DAU" icon={Users} loading={users.isPending} value={users.data ? num(users.data.totals.dau) : "-"} hint={users.data ? `MAU ${num(users.data.totals.mau)} · 고착도 ${percent(users.data.totals.stickiness)}` : usersNotReady ? "아직 준비 중인 통계예요" : undefined} />
        ...
        <StatCard label="p95 응답" icon={Timer} loading={recs.isPending} tone={t && t.p95_latency_ms !== null && t.p95_latency_ms <= 1800 ? "good" : "warn"} value={t && t.p95_latency_ms !== null ? `${(t.p95_latency_ms / 1000).toFixed(2)}초` : "-"} hint="목표 1.8초 이하" />
        ...
        <StatCard
          ...
          value={pendingCount !== undefined ? `${num(pendingCount)}${pendingIsFloor ? "+" : ""}` : "-"}
          hint={
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <Panel title="일별 활성 사용자" description="DAU 와 신규 가입">
          {users.isPending ? (
            ...
            <EmptyState size="sm" title="아직 준비 중인 통계예요" description="제품 분석 도구(PostHog·GA4)를 연결하면 DAU 와 신규 가입이 여기에 나와요." />
          ) : users.isError ? (
        ...
        <Panel title="코스 생성 · 저장 · 다시 짜기" description="하루 단위">
          ...
          ) : recs.data.daily === null ? (
            <EmptyState size="sm" title="아직 준비 중인 통계예요" description={`일별 추이는 아직 집계하지 않아요. 최근 30일 동안 코스 ${num(recs.data.totals.generated)}건이 만들어졌어요.`} />
