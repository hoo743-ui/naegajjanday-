...
const TOP_COLUMNS: Column<TopPlace>[] = [
  ...
  { key: "swap", header: "교체당함", align: "right", cell: (r) => `${num(r.swap_outs)} (${percent(r.impressions ? r.swap_outs / r.impressions : 0)})` },
];
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
        ...
        <StatCard label="한 곳 교체" icon={Shuffle} loading={q.isPending} value={t ? percent(t.swap_rate, 1) : "-"} />
        <StatCard label="평균 예산" icon={Wallet} loading={q.isPending} value={t ? won(t.avg_budget) : "-"} hint={t ? `평균 사용률 ${percent(t.avg_budget_utilization)}` : undefined} />
        <StatCard label="p95 응답" icon={Timer} loading={q.isPending} tone={t && t.p95_latency_ms <= 1800 ? "good" : "warn"} value={t ? `${(t.p95_latency_ms / 1000).toFixed(2)}초` : "-"} hint="목표 1.8초 이하" />
      </div>

      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <Panel title="일별 생성 · 저장 · 다시 짜기">
          {!d ? <Skeleton className="h-[260px] rounded-2xl" /> : d.daily.length === 0 ? <EmptyState size="sm" title="아직 생성된 코스가 없어요" /> : <TrendChart data={d.daily} xKey="date" label="일별 코스 생성, 저장, 다시 짜기 추이" series={[{ key: "generated", label: "생성" }, { key: "saved", label: "저장" }, { key: "rerolled", label: "다시 짜기" }]} />}
        </Panel>
        <Panel title="목적별 생성 수" description="막대에 마우스를 올리면 건수를 볼 수 있어요">
          {!d ? <Skeleton className="h-[260px] rounded-2xl" /> : d.by_purpose.length === 0 ? <EmptyState size="sm" title="목적별 데이터가 없어요" /> : <BarsChart horizontal data={d.by_purpose} xKey="purpose_name" label="목적별 코스 생성 수" series={[{ key: "generated", label: "생성" }]} xFormat={(v) => v} />}
        </Panel>
        <Panel title="예산 분포" description="요청 시 입력한 총 예산">
          {!d ? <Skeleton className="h-[260px] rounded-2xl" /> : d.budget_histogram.length === 0 ? <EmptyState size="sm" title="예산 데이터가 없어요" /> : <BarsChart data={d.budget_histogram} xKey="bucket" label="총 예산 구간별 요청 수" series={[{ key: "count", label: "요청 수", color: "#8E8BFF" }]} xFormat={(v) => v} />}
        </Panel>
        <Panel title="응답 지연" description="코스 생성 API (LLM 설명은 별도 스트림)">
          {!d ? <Skeleton className="h-[260px] rounded-2xl" /> : d.latency.length === 0 ? <EmptyState size="sm" title="지연 데이터가 없어요" /> : <TrendChart data={d.latency} xKey="date" label="일별 응답 지연 p50과 p95" format={(v) => `${Math.round(v)}ms`} series={[{ key: "p50", label: "p50" }, { key: "p95", label: "p95", color: "#E0457F" }]} />}
        </Panel>
      </div>
      ...
      <Panel title="많이 추천된 장소" description="교체당한 비율이 높으면 점수에 비해 만족도가 낮다는 신호예요." className="mt-5">
        <DataTable caption="많이 추천된 장소" columns={TOP_COLUMNS} rows={top.data?.items} rowKey={(r) => r.id} isLoading={top.isPending} error={top.error} onRetry={() => void top.refetch()} empty={{ title: "아직 추천된 장소가 없어요" }} />
      </Panel>
    </>
  );
}
