...
variable "domain_name" {
  description = "Domain of this environment, e.g. staging.naegajjanday.com. The API is served on api.<domain_name> and on <domain_name>/v1."
  type        = string
}
...
variable "opensearch_create_service_linked_role" {
  description = "true only for the first VPC OpenSearch domain in the AWS account."
  type        = bool
  default     = true
}
