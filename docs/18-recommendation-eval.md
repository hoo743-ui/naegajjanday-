...
| **예산이 코스를 설계하지 않는다**: 둘이서 30만 원을 줘도 7.6만 원(25%)만 쓰고 "디저트 하나 더". 10만 원 코스와 30만 원 코스가 같다 | `budget_probe` | 목적×시간대마다 "넉넉한" 템플릿(1인 45,000원~). 남은 돈이 30% 이상이면 다른 말을 한다 | 같은 조건 **4곳 · 7.6만 원(25%) → 7곳 · 16.4만 원(54%)**. 10만 원과 30만 원이 다른 하루가 된다. 평균가 가게로는 그 이상 쓸 수 없어, 이때 짠이는 "이 시간엔 164,000원이면 충분해요"라고 말한다 (후보·가중치를 바꾸기 전 중간 측정값은 73%였다 — 더 비싼 고깃집이 뽑혔을 때) |
